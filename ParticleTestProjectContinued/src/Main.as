package  
{
	...
	public class Main extends MovieClip
	{
		...
		private var collectionBox:Shape = new Shape();
		...
		public function onAddedToStage(event:Event):void
		{
			...
			spaceship.x = 275;
			spaceship.y = 200;
			this.thrusterParticle = new ThrusterParticles(spaceship.x - 50, spaceship.y - 50, 1000);
			stage.addChild(thrusterParticle);
			
			thrusterSpawnPoint = thrusterParticle.globalToLocal(spaceship.localToGlobal(new Point(thrusterSpawns.x + thrusterSpawns.width/2, thrusterSpawns.y + thrusterSpawns.height)));
		...
		public function onEnterFrame(event:Event):void
		{
			...
			if (UP_KEY) {
				...
				if (this.RIGHT_KEY || this.LEFT_KEY) {
					if (this.RIGHT_KEY && !this.LEFT_KEY) {
						if (this.oneFour % 4 == 0) {
							thrusterSpawnPoint = thrusterParticle.globalToLocal(spaceship.localToGlobal(new Point(secondThrusterSpawns.x + secondThrusterSpawns.width/2, secondThrusterSpawns.y + secondThrusterSpawns.height+2)));
						} else {
							thrusterSpawnPoint = thrusterParticle.globalToLocal(spaceship.localToGlobal(new Point(thrusterSpawns.x + thrusterSpawns.width/2, thrusterSpawns.y + thrusterSpawns.height+2)));
						}
			...
			if (RIGHT_KEY) {
				...
				if (RIGHT_KEY && UP_KEY) {
					...
					thrusterSpawnPoint = thrusterParticle.globalToLocal(spaceship.localToGlobal(
		...
		public function drawGraphics():void
		{
			...
			spaceship.graphics.moveTo(0, -6);
			spaceship.graphics.lineTo(3, -1);
			spaceship.graphics.lineTo( -3, -1);
			spaceship.graphics.lineTo(0, -6);
			spaceship.graphics.endFill();
			
			collectionBox.graphics.lineStyle(0.1, 0xFFFFFF);
			collectionBox.graphics.beginFill(0x000000, 1);
			collectionBox.graphics.drawRect(0, 0, 5, 8);
			collectionBox.graphics.endFill();
			
			collectionBox.x = -collectionBox.width/2;
			collectionBox.y = 1;
			
			spaceship.addChild(collectionBox);
			...
			secondThrusterSpawns.x = 2;
			secondThrusterSpawns.y = 12;
