package 
{
	...
	public class moreCoin extends MovieClip implements ICoin
	{	
		// Player Reference
		...
		// Constructor
		public function moreCoin(player:initPlayer)
		{			
			...
			this.removeTimer = new Timer(30000);
		...
		public function tooClose(coins:Array)
		{
			...
			for (var i:int = 0; i < coins.length; i++)
			{
				...
				if (this.x != coins[i].x && this.y != coins[i].y)
				{
					...
					if (Math.abs(this.y - coins[i].y) <= 12)
					{
						if (distance < 0)
						{
							distance = Math.abs(distance);
							// If collision occurs on the left decrease the xVel
							if (distance < 6 && this.xVel >= -0.4)
							{
								this.xVel -=  0.02;
							}
						}
