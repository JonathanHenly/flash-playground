package 
{
	...
	public class ThrusterParticles extends MovieClip 
	{
		...
		*/
		...
		public var MAX_SPEED:Number = 0.5;
		
		private const ANGLE_DIFFERENCE:Number = 10;
		
		public function ThrusterParticles(x:int, y:int, amount:int, type:int = 0):void
		{
		...
		{
			//Size of the bitmap:
			this.displayWidth = 550; // parent.width;
			this.displayHeight = 400; // parent.height;
		...
		public function addParticles(amount:int = 0):void
		{
			...
			while (newCount < amount) {
				if (tmpParticle.recycle) {
					...
					var spaceshipRotation = Main.spaceship.rotation;
					...
					tmpParticle.x = Main.thrusterSpawnPoint.x;
					tmpParticle.y = Main.thrusterSpawnPoint.y;
